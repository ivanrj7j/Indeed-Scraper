...
#         print('jj',self.keyword_list,self.location_list)
    ...
    def start_requests(self):
        for k in self.keyword_list:
            for l in self.location_list:
                ...
                yield scrapy.Request(indeed_jobs_url, callback=self.parse, meta={'keyword': k, 'location': l, 'offset': 0})

    def parse(self, response):
        #this function is used to parse through each job page
        jobs = response.xpath('//div[@class="job_seen_beacon"]')
        job_page_url = list(set(jobs.xpath('//div[@class="css-1m4cuuf e37uo190"]//a/@data-jk').getall()))#(Returns  a list  of all  jobs available in a page)
        Posted_DT = jobs.xpath('//span[@class="date"]/text()').get()
    ...
    def parse_job_page(self, response):
        ...
        if c_li is not None:
            ...
            c_key = None
        r_rating = main.xpath('//div[@class="css-1unnuiz e37uo190"]/div/@aria-label').get()
        ...
        yield {
            ...
            'Job_Location': main.xpath('//div[@class="css-6z8o9s eu4oa1w0"]/div/text()').get(),
            'Rating': r_rating,
            'Salary': response.xpath('//span[@class="css-2iqe2o eu4oa1w0"]/text()').get(),
    ...
    def parse_company_page(self, response):
        ...
        reviews_url = response.xpath('//li[@data-tn-element="reviews-tab"]//a/@href').get()

        # Fetch reviews asynchronously if reviews_url is not None
        # it crawls to review page and fetch relevant results
        if reviews_url:
            yield scrapy.Request(response.urljoin(reviews_url), callback=self.parse_review_page, meta={'company_data': company_data})
        else:
    ...
    def extract_company_data(self, response):
        ...
        company_description = self.tag_remover(company_description_xpath_result) if company_description_xpath_result else None
        ceo = response.xpath('//span[@class="css-1w0iwyp e1wnkr790"]/text()').get()
        ...
        founded = response.xpath('//div[@class="css-1w0iwyp e1wnkr790"]/text()').get()
        ...
        if c_size is not None:
            ...
            company_size= None
        revenue = response.xpath('//div[@class="css-1k40ovh e1wnkr790"]/text()').get()
        ...
        salaries_xpath = response.xpath('//div[@data-tn-section="salary-section"]/section[@class="css-125rq2h eu4oa1w0"]').get()
        ...
        qna_xpath = response.xpath('//div[@data-tn-section="qna-section"]').get()
        ...
        review_url = 'https:www.indeed.com' + response.xpath('//li[@data-tn-element="reviews-tab"]//a/@href').get()
    ...
    def extract_review_data(self, response):
        ...
        reviews_count = response.xpath('//div[@class="css-104u4ae eu4oa1w0"]/text()').get()
        reviews_by_category = response.xpath('//div[@class="css-xpga8e eu4oa1w0"]/div[@class="css-19hfeub eu4oa1w0"]/a/@aria-label').getall()
        reviewer_name = review_list.xpath('//div[@class="css-8a5o2x e1wnkr790"]//a/text()').get()
        reviews = review_list.xpath('//div[@data-tn-component="reviewDescription"]//span[@class="css-15r9gu1 eu4oa1w0"]/text()').get()
        reviews_po_date = review_list.xpath('//span[@class="css-xvmbeo e1wnkr790"]').get()
        if reviews_po_date is not None:
            reviews_posted_date = reviews_po_date.split(">")[-2].split('<')[0]
        else:
